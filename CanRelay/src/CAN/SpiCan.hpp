...
public:
    SpiCan(spi_device_handle_t device, CAN_SPEED speed, CAN_CLOCK clock, gpio_num_t interruptPin) : ACan()
    {
        ...
        ESP_ERROR_CHECK(MCPErrorToESPError(_mcp2515->setBitrate(speed, clock)));
        ...
        ESP_ERROR_CHECK(gpio_isr_handler_add(interruptPin, OnInterrupt, this));
    ...
    esp_err_t Send(SCanMessage message, TickType_t timeout = 0)
    {
        ...
        {
            //ESP_LOGV("twai", "SPI Write Timeout");
        ...
        {
            ESP_LOGE("spi", "SPI Write Error: %d", res);
        }
        // else
        // {
    ...
    esp_err_t Receive(SCanMessage* message, TickType_t timeout = 0)
    {
        ...
        {
            //ESP_LOGV("spi", "SPI Wait: %d, %d", uxSemaphoreGetCount(_interruptSemaphore), gpio_get_level(_interruptPin));
        ...
        else if (readResult != MCP2515::ERROR_OK)
        {
            ESP_LOGE("spi", "SPI Read Error: %d", readResult);
            return MCPErrorToESPError(readResult);
        }
    ...
    esp_err_t GetStatus(uint32_t* status, TickType_t timeout = 0)
    {
        ...
        if (xSemaphoreTake(_driverMutex, timeout) != pdTRUE)
        {
            //ESP_LOGV("twai", "SPI Status Timeout");
            return ESP_ERR_TIMEOUT;
        }
        #endif
