...
MCP2515::MCP2515(spi_device_handle_t *s)
{
    ...
    // buscfg.miso_io_num=MOSI_PIN;
    // buscfg.mosi_io_num=MISO_PIN;
    ...
    // devcfg.clock_speed_hz=SPI_CLOCK_SPEED,               //Clock out at 1 MHz
    // devcfg.mode=0,                                   
    // devcfg.spics_io_num=CS_PIN,                          //Chip select
    // devcfg.queue_size=7,
                           
    // //Initialize the SPI bus
    // ret=spi_bus_initialize(SPI_BUS_HOST, &buscfg, SPI_DMA_CHAN);
    // assert(ret==ESP_OK);
    // //Attach the MCP2515 to the SPI bus
    // ret=spi_bus_add_device(SPI_BUS_HOST, &devcfg, &*spi);
    // assert(ret==ESP_OK);
    // // ESP32SIM800 END 
}
